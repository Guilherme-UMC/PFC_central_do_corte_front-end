...
// Card de barbearia com endereço completo + telefone
...
const Home = () => {
  ...
  // Consome o estado de busca do RootLayout — mesma instância que a Navbar usa
  ...
  // Carrega serviços de todas as barbearias quando há filtro de serviço ativo
  useEffect(() => {
  ...
  // Ao clicar em AGENDAR: cliente logado → dashboard de agendamento; senão → login
  const handleAgendar = (barbeariaId) => {
    if (isAuthenticated && isCliente) {
      navigate('/dashboard/cliente', { state: { barbeariaId } });
    } else if (isAuthenticated) {
      // Outro role (funcionário, adm): só redireciona para login para criar conta cliente
      navigate('/login');
    } else {
      navigate('/login');
    }
